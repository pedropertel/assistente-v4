...
- 🟡 Banco (Fase 2 em andamento — **7 de 9 tarefas + 2 evolutivas (2.5.1, 2.6.1) ≈ 78% da fase**): **11 tabelas reais** — `entidades` (6 regs), `tarefas` (kanban), `eventos` (agenda + sync Google), `pastas` (hierarquia 3 níveis), `documentos` (+ bucket Storage privado), `agentes` (1 reg: "Assistente"), `personas` (5 regs incl. Roteador interno), `chat_mensagens` (lista plana com observabilidade completa), `chat_anexos` (4 tipos, **única CASCADE registrada**), **`sitio_categorias`** (hierarquia 2 níveis, 29 seeds editáveis), **`sitio_lancamentos`** (financeiro com rastreio voz→lançamento via `transcricao_original` + `mensagem_origem_id`, valores em centavos bigint, datas sem hora). **6 FKs (2.5)** conectam tarefas/eventos/documentos aos agentes/personas via SET NULL. **Router pattern (2.5.1)** decide modelo por mensagem. **REGRA 12 (2.6.1):** customização total — todo seed = ponto de partida editável, soft-delete é padrão. Funções genéricas: `set_updated_at`, `validar_pasta_coerencia`, `validar_documento_pasta`. Convenções em `050 - Banco de Dados/CONVENÇÕES.md` (fuso, idempotência ALTER TABLE, FKs RESTRICT/SET NULL/CASCADE-exceção, naming, Storage + prefixo-exceção, Router pattern, customização total, soft-delete padrão, denormalização consciente). Faltam 2.8 (cedtec/Meta) e 2.9 (configurações).
