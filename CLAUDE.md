...
- 🟡 Banco (Fase 2 em andamento — **6 de 9 tarefas + 1 evolutiva (2.5.1) ≈ 67% da fase**): **9 tabelas reais** — `entidades` (6 regs), `tarefas` (kanban), `eventos` (agenda + sync Google), `pastas` (hierarquia 3 níveis), `documentos` (+ bucket Storage privado), `agentes` (1 reg: "Assistente"), `personas` (5 regs incl. Roteador interno), **`chat_mensagens`** (lista plana com observabilidade completa — tokens/custo USD+BRL/latência/papel/erro/favorita), **`chat_anexos`** (4 tipos, vínculo opcional com documentos, **única tabela com `ON DELETE CASCADE`** — exceção registrada em CONVENÇÕES). **6 FKs (Tarefa 2.5)** conectam `tarefas/eventos/documentos` aos `agentes/personas` via `ON DELETE SET NULL`. **Router pattern (Tarefa 2.5.1):** persona Roteador (Haiku) classifica cada mensagem; Edge Function da Fase 3 escolhe modelo via `modelo_override` ou mapeamento `nivel_complexidade → Haiku/Sonnet/Opus`. Bucket `documentos` Storage validado, prefixo `chat_anexos/` compartilha o mesmo bucket (exceção consciente registrada). Funções genéricas: `set_updated_at`, `validar_pasta_coerencia`, `validar_documento_pasta`. Convenções em `050 - Banco de Dados/CONVENÇÕES.md` (fuso, idempotência ALTER TABLE, FKs RESTRICT/SET NULL/CASCADE-exceção, naming, Storage + exceção do prefixo, Router pattern). Faltam 2.7 (sítio), 2.8 (cedtec/Meta), 2.9 (configurações).
