...
- 🟡 Banco (Fase 2 em andamento — **5 de 9 tarefas concluídas, 55%**): **7 tabelas reais** — `entidades` (6 regs), `tarefas` (kanban), `eventos` (agenda + sync Google), `pastas` (hierarquia 3 níveis), `documentos` (+ bucket Storage privado), `agentes` (1 reg: "Assistente"), `personas` (4 regs: Marcos/Bruno/Marcela/Alemão como modos de adaptação). **6 FKs novas (Tarefa 2.5)** conectam `tarefas/eventos/documentos` aos `agentes/personas` via `ON DELETE SET NULL`. Bucket `documentos` no Storage validado. Bucket `agentes` antigo apagado (tech debt resolvido). Funções genéricas: `set_updated_at`, `validar_pasta_coerencia`, `validar_documento_pasta`. Convenções em `050 - Banco de Dados/CONVENÇÕES.md` (fuso, idempotência, FKs estruturais vs metadados, ALTER TABLE idempotente, naming, Storage). Faltam tarefas 2.6–2.9 (chat_mensagens, sítio, cedtec, configurações).
