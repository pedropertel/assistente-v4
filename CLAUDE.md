...
- ✅ **Fase 3 em andamento — 3/9 sub-fases (3.0 ✅, 3.A ✅, 3.B ✅), próxima: 3.C.**
...
- **Próximo:** Fase 3.C — `prompt_base` real + placeholders + histórico de 20 mensagens. **Plano oficializado em 2026-05-02 após terceiro triplo /plan** (5 sub-tarefas: 3.C.0 UPDATE SQL pra adicionar placeholders no prompt do banco, 3.C.1 lookup do agente inteiro + remoção de constantes hardcoded, 3.C.2 helpers de substituição com `{usuario}/{data_hora}/{entidade_atual}/{persona_ativa}`, 3.C.3 histórico de 20 msgs como `messages[]`, 3.C.4 docs/fechamento). 4 achados factuais via REGRA 11 registrados (prompt_base sem placeholders hoje, max_tokens vai de 1024 → 4096, configuracoes não tem `historico_max_mensagens` ainda). Detalhamento ativo no topo de `.claude/plans/temporal-tinkering-castle.md`. **Caminho curto até Marcos em produção:** ~17.5h restantes (3.C → 3.D → 3.F). Total Fase 3 completa: ~34.5h restantes. Decisões críticas: Web Speech API pra voz (Whisper fica como fallback futuro opcional), Anthropic `tools` parameter nativo + observabilidade via `chat_mensagens.tool_calls/tool_results jsonb` (ALTER TABLE na 3.F.0.5), bootstrap Meta via SQL manual (dívida temporária), streaming SSE depois de Marcos.
