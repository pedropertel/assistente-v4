...
- 🟡 Banco (Fase 2 em andamento — **8 de 9 tarefas + 2 evolutivas (2.5.1, 2.6.1) ≈ 89% da fase**): **16 tabelas reais** — `entidades`, `tarefas`, `eventos`, `pastas`, `documentos` (+ bucket Storage), `agentes`, `personas` (incl. Roteador), `chat_mensagens`, `chat_anexos` (1ª CASCADE), `sitio_categorias`, `sitio_lancamentos` (com rastreio voz→lançamento), e **5 do módulo Meta Ads (2.8)**: `meta_credenciais` (token via Supabase Vault), `meta_conexoes` (multi-conta entidade↔ad_account, saldo cacheado ~5min), `meta_campanhas_cache` + `meta_adsets_cache` + `meta_ads_cache` (3 níveis com `raw_data jsonb` pra métricas obscuras, FK lógica entre níveis, **3 novas CASCADE** registradas — total 4 exceções consolidadas). **Router pattern (2.5.1)**, **REGRA 12 (2.6.1)** customização total. Convenções em `050 - Banco de Dados/CONVENÇÕES.md`: fuso, idempotência ALTER TABLE, FKs RESTRICT/SET NULL/CASCADE-exceção, naming, Storage + prefixo-exceção, Router pattern, customização total, soft-delete padrão, denormalização consciente, **Integração com sistemas externos (Vault + cache)**. Falta apenas **2.9 (configurações)** pra fechar a Fase 2.
