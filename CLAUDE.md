...
- 🟡 Banco (Fase 2 em andamento — **5 de 9 tarefas + 1 evolutiva (2.5.1)**): **7 tabelas reais** — `entidades` (6 regs), `tarefas` (kanban), `eventos` (agenda + sync Google), `pastas` (hierarquia 3 níveis), `documentos` (+ bucket Storage privado), `agentes` (1 reg: "Assistente"), `personas` (5 regs: Marcos/Bruno/Marcela/Alemão visíveis + Roteador interno via 2.5.1). **6 FKs (Tarefa 2.5)** conectam `tarefas/eventos/documentos` aos `agentes/personas` via `ON DELETE SET NULL`. **Router pattern (Tarefa 2.5.1):** persona Roteador (sempre Haiku) classifica cada mensagem; Edge Function da Fase 3 escolhe modelo final via `modelo_override` ou mapeamento `nivel_complexidade → Haiku/Sonnet/Opus`. Bucket `documentos` no Storage validado. Bucket `agentes` antigo apagado. Funções genéricas: `set_updated_at`, `validar_pasta_coerencia`, `validar_documento_pasta`. Convenções em `050 - Banco de Dados/CONVENÇÕES.md` (fuso, idempotência ALTER TABLE, FKs estruturais vs metadados, naming, Storage, **Router pattern**). Faltam tarefas 2.6–2.9 (chat_mensagens, sítio, cedtec, configurações).
